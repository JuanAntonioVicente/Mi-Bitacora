...
app.get("/entradas", async (req, res) => {
    const resultado = await pool.query("SELECT * FROM entradas");
    res.json(resultado.rows);
});
...
app.post("/entradas", async (req, res) => {
    ...
    const peticion = await pool.query("INSERT INTO entradas (listaid, fecha, nombre, tiempo, puntuacion) VALUES ($1, $2, $3, $4, $5) RETURNING *",
...
app.put("/entradas/:id", async (req, res) => {
    ...
    const tiempo = req.body.tiempo;
    const puntuacion = req.body.puntuacion;
    const modificar = await pool.query("UPDATE entradas SET fecha = $1, nombre = $2, tiempo = $3, puntuacion = $4 WHERE id = $5 RETURNING *",
